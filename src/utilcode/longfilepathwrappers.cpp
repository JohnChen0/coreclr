...
private:   
#ifndef FEATURE_PAL
        static SString ExtendedPrefix;
        static SString DevicePathPrefix;
        static SString UNCPathPrefix;
        static SString UNCExtendedPathPrefix;
...
public:
        ...
        static BOOL IsPathFormatRelative(SString & path);
        ...
        )
{
    ...
    EX_TRY
    {
        ...
        if (LongFile::IsPathFormatRelative(path) || SUCCEEDED(LongFile::NormalizePath(path)))
        {
            //Adding the assert to ensure relative paths which are not just filenames are not used for LoadLibrary Calls
            _ASSERTE(!LongFile::IsPathFormatRelative(path) || LongFile::ContainsDirectorySeparator(path)); 
        ...
        )
{
    ...
    EX_TRY
    {
        ...
        {
            COUNT_T len = lpBuffer.GetUnicodeAllocation();
            ret = SearchPathW(
                    lpPath,
                    lpFileName,
                    lpExtension,
                    len,
                    lpBuffer.OpenUnicodeBuffer(len - 1),
                    lpFilePart
                    ); 
        ...
        )
{
    ...
    EX_TRY
    {
        ...
        {
            COUNT_T len = lpszShortPath.GetUnicodeAllocation();
            ret = GetShortPathNameW(
                    longPath.GetUnicode(),
                    lpszShortPath.OpenUnicodeBuffer(len - 1),
                    (DWORD)len
                    );
        ...
        )
{
    ...
    EX_TRY
    {
        ...
        {
            COUNT_T size = lpszLongPath.GetUnicodeAllocation();
    ...
    )
{
    ...
    EX_TRY
    {
        COUNT_T size = buffer.GetUnicodeAllocation();
        ...
        if (ret > size)
        {
            ...
            ret = GetModuleFileNameW(
                hModule,
                buffer.OpenUnicodeBuffer(ret - 1),
                ret
                );
        }
    ...
    )
{
    ...
    EX_TRY
    {
        ...
        ret  = GetTempFileNameW(
            ...
            lpPrefixString,
            uUnique,
            lpTempFileName.OpenUnicodeBuffer(size - 1)
            );

        lastError = GetLastError();
        lpTempFileName.CloseBuffer(ret);
    }
    EX_CATCH_HRESULT(hr);
    ...
    )
{
    ...
    EX_TRY
    {
        ...
        COUNT_T size = lpBuffer.GetUnicodeAllocation();
        ...
        )
{
    ...
    EX_TRY
    {
        LongPathString TargetFileName(LongPathString::Literal, lpTargetFileName);
        LongPathString linkFileName(LongPathString::Literal, lpSymlinkFileName);

        if (SUCCEEDED(LongFile::NormalizePath(TargetFileName)) && SUCCEEDED(LongFile::NormalizePath(linkFileName)))
        {
            ret = CreateSymbolicLinkW(
                    linkFileName.GetUnicode(),
                    TargetFileName.GetUnicode(),
                    dwFlags
                    );
        ...
        )
{
    ...
    EX_TRY
    {
        LongPathString path(LongPathString::Literal, lpApplicationName);
...
SString LongFile::ExtendedPrefix(SString::Literal, W("\\\\?\\"));
SString LongFile::DevicePathPrefix(SString::Literal, W("\\\\.\\"));
SString LongFile::UNCExtendedPathPrefix(SString::Literal, W("\\\\?\\UNC\\"));
SString LongFile::UNCPathPrefix(SString::Literal, W("\\\\"));
...
BOOL LongFile::IsPathFormatRelative(SString & path)
{
    if (path.GetCount() < 2) 
    { 
...
HRESULT LongFile::NormalizePath(SString & path)
{
    ...
    //if (!IsPathFormatRelative(path) && path.GetCount() < MAX_PATH)    //REMOVE: BEFORE FINAL CHECKIN
    //    return S_OK;
    ...
    COUNT_T size = path.GetUnicodeAllocation();
